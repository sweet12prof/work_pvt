...
void GlobalAnalyser_FT::fill_graph(std::vector<TraceData>& traceChunk,
  ...
  for (UINT64 i{0}; i < traceChunk.size(); i++) {
    ...
    if (value_to_vertex.find(value_property) == value_to_vertex.end()) {
      auto vertex = boost::add_vertex({value_property, 1}, g);
      value_to_vertex[value_property] = vertex;
    } else {
      g[value_to_vertex[value_property]].value_frequency++;
    }
  }
  ...
  for (size_t i = 0; i + 1 < traceChunk.size(); ++i) {
    ...
    auto curr_vertex = value_to_vertex[curr_property];
    auto next_vertex = value_to_vertex[next_property];

    auto [e, exists] = boost::edge(curr_vertex, next_vertex, g);

    if (!exists) {
      bool inserted;
      boost::tie(e, inserted) = boost::add_edge(curr_vertex, next_vertex, g);
